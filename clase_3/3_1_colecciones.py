...
# mi_lista.extend([60, 70]) # [10, 20, 30, 40, 60, 70] 
# mi_lista_2 = [2] + [5, 6]
# print(f"Después de extend([60, 70]): {mi_lista}")

# # insert(i, x): Inserta un ítem en la posición dada.
# mi_lista.insert(100, 25)  # Inserta 25 en la posición 2 (elemento, posicion/indice)
...
# indice = mi_lista.index(2) # 1 .index(2, 5, 10)
...
# mi_lista.sort() # float, int, string / mi_lista.sort(reverse=True)
...
# difference(*others): Devuelve la diferencia de conjuntos. (lo que sobra en un conjunto respecto al otro.)
A = {1, 2, 3, 4}
B = {3, 4, 5, 6}
...
# # symmetric_difference(other): Devuelve la diferencia simétrica. (lo que no comparten los conjuntos.)
...
nombre = mi_diccionario.get("nombre") # null
...
par_eliminado = mi_diccionario.popitem() # ("edad", 30)
